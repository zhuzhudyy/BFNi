...
from contextual_bo_model import ContextualBayesianOptimizer, select_scheme, SCHEME_TARGETS, DEFAULT_PROCESS_BOUNDS
...
if __name__ == "__main__":
    ...
    print("\n[2/2] 正在绘制工艺参数响应面图 (2D等高线 + 3D曲面)...")
    # 这里默认绘制 退火温度(Temp) 和 保温时间(Time) 的关系
    # 传入target_scheme确保响应面针对特定晶向
    plot_2d_landscape(optimizer, data_file, output_dir, 
                      param_x='Process_Temp', param_y='Process_Time',
                      target_scheme=target_scheme)
    
    print(f"\n可视化分析完成！所有图片已保存至文件夹: {output_dir}/")
    print("  - 包含: Parity Plot (LOOCV验证)、2D等高线图、3D曲面图")