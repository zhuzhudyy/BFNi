...
class ContextualBayesianOptimizer:
    ...
    def train(self, csv_path):
        ...
        # 自动识别初始状态特征列
        self.pre_feature_cols = [col for col in df.columns if col.startswith('Pre_')]
        
        # 构建特征矩阵 X 与目标向量 y
        X_df = df[self.pre_feature_cols + self.process_cols]
        ...
        print(f"代理模型训练完毕。吸收样本量: {len(df)}，状态特征维度: {len(self.pre_feature_cols)}。")
        print(f"优化后核函数参数: {self.gpr.kernel_}")

    def expected_improvement(self, X_process_array, x_pre_array, y_best):
        """
        计算期望提升 (Expected Improvement, EI) 采集函数
        """
        n_samples = X_process_array.shape[0]
        # 将静态的预处理特征与遍历的工艺参数进行拼接
        X_concat = np.hstack([np.tile(x_pre_array, (n_samples, 1)), X_process_array])
    ...
    def recommend_next_process(self, new_pre_features, n_random_starts=100000):
        """
        给定新样品的初始特征，推荐全局最优退火工艺
        """
        ...
        # 评估所有采样点的 EI 值
        ei_values, mu_values, sigma_values = self.expected_improvement(sampled_process, x_pre, y_best)
